...
> **Update, 2026-07-20 (M4 integration to `main`):** the scheduler decision below is now
> **RESOLVED** — a durable job queue was built and merged (as "C-infra", split out from this
> feature) rather than the external-cron option this doc recommended. Read
> `docs/superpowers/handoffs/2026-07-14-c-infra-shipped-next-c-recurring.md` first — it
> supersedes the "gating decision" and "migration sketch" sections below on those two points
> (scheduler + migration number) and carries the reaper blocker that must land before
> C-recurring starts. Slices A/A-UI/B and C-infra are all merged to `main`; branch C-recurring
> off `main`. The rest of this doc (open questions, sketch of `src/recurring/` domain shape)
...
- **A — money-in loop** — shipped, **merged to `main` 2026-07-20** (was PR #2
  `m4a-ar-money-in-loop`): open-item tracking on outbound `einvoices`
  (`status`/`due_date`/`amount_paid_cents`), `settleReceivable`, `arAging`, invoice-linked bank
  matching, per-customer `payment_terms_days`.
- **A-UI** — shipped, merged 2026-07-20 (was PR #3 `m4a-ui-settle`, stacked on #2): `/invoices`
  payment columns + settle/void drawer.
- **B — dunning + late fees** — shipped, merged 2026-07-20 (was PR #4 `m4b-dunning`, stacked on
  #2): `src/dunning/` per-client policy + escalation stages + informational late fees +
  `runDunning` emitting bookkeeper tasks, now driven by the job queue (`dunning_run` handler)
  instead of manual-trigger-only.
- **C-infra** — shipped, merged 2026-07-20 (grew out of this handoff's scheduler question, built
  and merged as its own thing): the durable job queue. See the 2026-07-20 banner above.
- **C — recurring invoices (THIS)** — ⛔ still not started; this is the next open slice.
...
**Branching:** PR #2 (slice A), PR #3 (A-UI), and PR #4 (slice B + C-infra) are all merged to
`main` as of the 2026-07-20 M4 integration — **branch slice C off `main` now.** (This
paragraph originally said to branch off `m4a-ar-money-in-loop` pending PR #2; that merge has
since happened, so `main` already has slice A's `sendInvoice` issue-path changes, the
receivables read model, and the job-queue infra C-recurring will ride.)

## The gating decision: a scheduler — RESOLVED (see 2026-07-20 update banner above)

> This section is kept for historical context (the options considered and why). The actual
> outcome was **not** Option 1 below — it was Option 2, built as its own "C-infra" spec/plan
> and merged to `main`. See
> `docs/superpowers/handoffs/2026-07-14-c-infra-shipped-next-c-recurring.md` for what shipped:
> a durable Postgres `jobs` table + `bookkeeping_worker`/`bookkeeping_supervisor` roles +
> handler registry (`src/jobs/handlers.ts`) + `drainOnce` (`src/jobs/worker.ts`) + a chain
> reaper, drained in production via `GET /api/cron/jobs-drain` (Vercel cron, 06:00 daily).
> Dunning (`dunning_run` in `src/jobs/register.ts`) is already the first consumer,
> self-perpetuating one day at a time. **C-recurring's job is to add a `recurring_generate`
> handler to `src/jobs/register.ts` that mirrors that pattern** — no new scheduler infra to
> build, just a new handler + the domain logic below.
...
> **2026-07-20 update:** migration numbers below are stale — slices A/B renumbered to
> **037–041** on `main` (receivables `037`, dunning `038`, jobs `039`, dunning-jobs-backfill
> `040`, supervisor-role `041`), and the 2026-07-20 M4 integration added **`042`**
> (credit-note applications, unrelated to slice C). **Next free migration number for
> C-recurring is `043`**, not `034`. The RLS-mirroring guidance (point at `037_receivables.sql`
> instead of `032_receivables.sql`) and the reuse-`dunning_run`-pattern guidance still stand —
> see the C-infra handoff for the exact `src/jobs/register.ts` shape to mirror.

- **Migration `043`** (highest existing on `main` is `042_credit_note_applications.sql`). Full
  RLS mirroring `037_receivables.sql`/`012_autonomy_policy.sql` (ENABLE+FORCE,
  `_tenant_isolation` policy on `current_setting('app.current_client_id', true)::uuid`, grants
  to `bookkeeping_app`).
...
## Also fold in while wiring the shared cron (slice B pre-scheduler follow-ups) — DONE

> **2026-07-20 update:** both items below shipped as part of C-infra, nothing left to do here.
> Per the C-infra handoff: `dunning_events` insert is now event-first
> `ON CONFLICT DO NOTHING RETURNING`, and the dunning routes reject negative days/bps/flat
> (clean 400s). Kept here only so C-recurring doesn't rediscover the same follow-ups against a
> stale mental model.
