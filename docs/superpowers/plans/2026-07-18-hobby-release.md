...
export async function GET(req: NextRequest, ctx: { params: Promise<{ token: string }> }) {
  ...
  if (!(await checkLoginAllowed(ids, at))) return NextResponse.json({ error: 'not found' }, { status: 404 });
  const preview = await previewInvite(token, at);
  if (!preview) {
    await recordLoginFailure(ids, at); // token probing burns the same budget
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ token: string }> }) {
  ...
  try {
    ...
    await recordLoginFailure(ids, at);
