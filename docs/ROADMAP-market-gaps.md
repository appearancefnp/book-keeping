...
loop, dunning, aging, and recurring invoices are shipped; only quotes→invoice remains, itself
...
2026-07-29, completing the set). M4 (AR lifecycle, 🔶) rounds out invoicing — C-recurring's
backend shipped with the 2026-07-20 merge and its last gaps (approval-dispatch fix, `/invoices`
Recurring tab, composer mode) closed 2026-07-31; slice D (quotes→invoice, customer statements)
remains the only open M4 work. M9 (VAT completeness) is no longer
