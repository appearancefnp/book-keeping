...
| # | Gap | Status | Notes |
|---|-----|--------|-------|
| M1 | **Financial statements** — Profit & Loss / Income Statement, Balance Sheet, Cash-Flow statement, Statement of Equity, on demand for any period | ⛔ | Only `trialBalance()` exists. Distinct from the statutory *annual report* (`HANDOFF §5`, §6.8) — this is the management P&L/BS people expect on a button, and should not wait for year-end closing. **Highest-value core gap.** New `src/reports/` over existing journal + chart of accounts; no migration needed to start. |
| M2 | **Accounts payable / vendor bills** — enter supplier bills, track what's owed, schedule/batch pay, AP aging | ⛔ | We have AR (invoicing) but no money-*out* workflow at all. Bills should reuse the parties + proposal + ledger substrate; pay runs feed the existing `pain.001` payment-order composer in `/bank`. |
...
| M5 | **Aged receivables / payables reports** | ⛔ | `/overview` shows a receivables total but no aging buckets. Trivial once M1's report layer exists; pairs with M2 for AP aging. |
...
| M7 | **Credit notes** | ⛔ | Already flagged (`HANDOFF §3`, needs UBL CreditNote type). Listed here because it blocks basic AR correction and is small scope / high embarrassment. |
