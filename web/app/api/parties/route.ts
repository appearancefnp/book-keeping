...
export async function POST(req: NextRequest) {
  ...
  const body = (await req.json().catch(() => ({}))) as {
    clientCompanyId?: string; kind?: PartyKind; name?: string; regNo?: string | null; vatNo?: string | null; countryCode?: string; paymentTermsDays?: number | null;
  };
  ...
  try {
    ...
    const result = await withTenant(ctx, (tx) =>
      createParty(tx, ctx, { kind: body.kind!, name: body.name!, regNo: body.regNo ?? null, vatNo: body.vatNo ?? null, countryCode: body.countryCode, paymentTermsDays: body.paymentTermsDays ?? null }),
    );
