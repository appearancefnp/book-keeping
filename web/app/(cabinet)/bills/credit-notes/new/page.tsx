...
function NewCreditNoteInner() {
  ...
  const [correctedBillNumber, setCorrectedBillNumber] = useState('');
  ...
  useEffect(() => {
    ...
    // included — the server's ?kind=vendor filter exact-matches and would drop them. Matches bills/new.
  ...
  const submit = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/vendor-credit-notes', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          clientCompanyId: client, vendorPartyId, creditNoteNumber, issueDate, currency: 'EUR',
          correctedBillNumber: correctedBillNumber.trim() ? correctedBillNumber.trim() : null, lines,
        }),
      });
      if (!res.ok) {
  ...
  }, [client, vendorPartyId, creditNoteNumber, issueDate, correctedBillNumber, lines, router, t]);

  const valid = !!vendorPartyId && !!creditNoteNumber.trim() && lines.length > 0 && lines.every((l) => l.description.trim());
    ...
    <div className={styles.page}>
      <main className={styles.main}>
        ...
        <div className={styles.fields}>
          ...
          <label className={styles.field}>
            {t('bills.creditNote.correctedBill')}
            <input value={correctedBillNumber} onChange={(e) => setCorrectedBillNumber(e.target.value)} />
          </label>
        </div>
...
export default function NewCreditNotePage() {
  return (
    <Suspense fallback={<NewCreditNoteSkeleton />}>
      <NewCreditNoteInner />
    </Suspense>
  );
}
